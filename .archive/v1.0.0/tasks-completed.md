# omni-me — tasks completed, archived at v1.0.0

The complete `tasks.md` as it stood at the **v1.0.0** tag (2026-08-31), kept verbatim: all
81 finished tasks across Phases 1–6, the full release roadmap (steps 1–9) with its findings,
every resolved friction-log entry, and the verification notes attached to each.

This is the *whole* file, not just the finished parts — an archive that dropped the open
items would be a worse record, and the live `tasks.md` is the place to read what is still
open. Expect overlap: anything still unresolved appears in both.

Treat everything here as **historical**. Claims were true when written; several were
superseded later in Cycle 4 without the original text being revised.
...
Do the current step, stop, let the user compact. Do NOT run ahead.*
  ...
  **A. Feature-completion (one push each):**
  ...
  2. **#370** — desktop freezes on FIRST open after a full reset (not on subsequent opens); editor
     ...
     opens fine; (b) cold-open pointed at the real box (`omni-box-example:3000`, health 200) → opens
  ...
  **D. Data catch-up (clean data BEFORE the wipe):**
  7. **Bring imports current** — the ledger import is months stale; use the **paisa process** to get it
     ...
     **✅ DONE 2026-08-30 — both halves landed and validated; the real write is deliberately deferred
     to step 8 (see the closing block).** Nothing here re-opens the
     ...
     **✅ BOTH DATA HANDOFFS LANDED + VALIDATED 2026-08-30 (second session that day).** Ledger
     regenerated by the user through **2026-08-28**; vault replaced with a fresh download through
     **2026-08-30** (old copy moved to `~/Documents/archive/`).
     ⚠️ **The new vault download nests one level** — the real root has the directory name repeated,
     and the outer level holds only that plus a stray `Attachments/`. `OMNI_VAULT` must point at the
     inner path; the outer one walks the wrong tree. Recorded in `.reference/` + the private
     `examples/README.md`, which also gained the archived-copy path.
     **Vault delta (dry-run tool, read-only):** 1352 journal / 343 generic / 4 demoted / 19 scan
     errors, walked 1714, dates `2022-07-22 .. 2026-08-30`. Against the archived imported copy (1294
     journal, ending 2026-07-03) that is **+58 contiguous daily notes and 0 dates lost** — a strict
     superset, so it can simply replace the old one. Generic / demoted / scan-error counts and the
     single rescued bad-frontmatter note are **identical** across both copies → the delta is purely new
     daily notes, **no new data issues to triage**.
     **Full dry-run import into a throwaway DB — all four gates pass:** `parsed 10582 transactions,
     0 parse errors, 0 balance failures`, committed 10582 / skipped 0; journal side reproduced the
     tool's counts exactly with **0 read errors**; balances **exact, 0 diffs** vs the `ledger bal`
     oracle; tag-anchor fidelity `ALL ANCHORS MATCH` after refreshing three stale expectations.
     **The three anchor "failures" were stale June-corpus constants, not import defects** — each
     re-derived from the `ledger` oracle first and only then updated in the private probe; two of the
     three also match the anchors in the user's own paisa `NEXT.md`. Numbers now live as assertions in
     the private `examples/README.md` § Expected output, for step 8 to reproduce.
     **New reconciliation pitfall, recorded so it isn't mistaken for a parser bug:** `ledger bal
     --flat` reports **inclusive** totals (`--flat` changes the display from a tree to full names; it
     does not stop a parent rolling up its children), while `core::ledger::balances` reports
     **exclusive** per-account balances. One account in the real tree has both its own postings and
     children, so a naive diff shows a spurious 2-row mismatch. Subtract each displayed account from
     its nearest displayed ancestor before comparing — the oracle's 112 rows then reconcile against
     the app's 111 with 0 diffs. Full detail in the private `examples/README.md` § Gotchas.
     **Why `local.db` was NOT written (user decision, 2026-08-30):** idempotency is asymmetric —
     financial re-imports skip on the content-hash `txn_id` and journal entries UPSERT by date, but
     generic notes mint a fresh ULID per run and would **duplicate all 343**. A clean vault import
     therefore requires removing `local.db` first — and a wiped `local.db` would immediately re-pull
     the box's old data on the next app open, undoing the clean slate. So the destructive clean write
     belongs **with** the box wipe, as one stretch, in step 8.
     **Also cleared en route:** the sitting-1 Critical explicitly flagged *"verify before roadmap
     steps 7–8"* (pull cursor on the server clock vs an author-clock pull filter) is **fixed** —
     `[review] key sync cursors on receipt, not author clock`.
     **Carry into step 8:** the app's current `budget.journal` holds a handful of transactions dated
     past the old ledger cut-off — app-created or auto-imported rows that a `local.db` wipe drops.
     Small, but name it before wiping rather than after.
...
- [x] **Boot shows the WRONG page building, then corrects — add a splash that holds until the app is
  ...
  **✅ ON-DEVICE VERIFIED 2026-08-30 — and the device found three defects the web pass could not.**
  Local release APK (throttled profile), debug-keystore signed so it installed over the existing app
  with no wipe, on the test phone (Samsung SM-G960W, Android 10 / API 29, Adreno 630). Method:
  `screenrecord --bugreport` (per-frame timestamp overlay) → 10fps frames → a colour fingerprint per
  frame, since the splash has an exact signature — its off-white core dot sits at the precise screen
  centre, so *centre bright + frame mean dark* is splash and *frame mean bright* is a white flash.
  `am start -W` reported `LaunchState: COLD` on every run. Harness + frames: this session's
  scratchpad; the reduction script is `analyze.sh`.
  1. **The override did not build at all.** `res/values/themes.xml`'s comment cited the CSS token
     with its literal double-hyphen prefix, and **XML forbids `--` inside a comment**; aapt enforces
     it, so `:app:mergeUniversalReleaseResources` failed the whole build. The previous session's
     "verified the override landed in `gen/android/`" was a *copy* check — `build.rs` only does
     `std::fs::copy`, and nothing parses the file until gradle, which had never run since. Comment
     rewritten, plus an in-file warning not to reintroduce it.
  2. **The splash sat for the full 6s deadline on every tab except Journal.** Measured Finances
     6300ms and 6400ms on two runs, Settings 6300ms, against Journal 2100ms cold / 300ms warm —
     i.e. the feature made boot *worse* for four of five tabs, including the one the user actually
     closes on. **Root cause:** the render gate opens on `is_loaded()`, but `active_tab` is restored
     by a *separate* future polling at 20ms, so the default `JournalPage` still mounts for one frame
     first — it is simply hidden behind the splash now. It takes a boot hold, the restore then
     unmounts it, and `use_boot_hold` had **no unmount cleanup**, so `pending` never returned to 0
     and the lift loop waited out the whole wall-clock cap. Journal was the only unaffected tab
     because it is the default and never gets swapped. Confirmed by *prediction before fixing*
     (per [[feedback-verify-hypothesis-before-build]]): the mechanism says any non-default tab, so
     Settings was tested and came back 6300ms. Fixed with a `use_drop` that releases the hold
     (`main.rs::use_boot_hold`). After: Finances 300ms, Settings 900ms, Journal 300ms, each landing
     on the correct tab with content built and no Journal frame.
  3. **A ~300ms full-white flash** between the charcoal native window and the splash, on every tab.
     `themes.xml` cannot reach it: the flash is the *document's* canvas, not the window's. The
     served `index.html` is generated by dx with no stylesheet link and no background of its own
     (Tailwind arrives later, injected by the wasm bundle), and Chromium's default canvas is white.
     Fixed **without** touching the deferred custom `index.html`: `MainActivity.onWebViewCreate`
     (wry's own hook, called from `WryActivity.setWebView` at creation, before the first load) sets
     `webView.setBackgroundColor(#1e1e1e)`, so an unstyled document paints charcoal. That hex now
     appears in three places on purpose — `themes.xml`, `MainActivity`, `--color-bg` — and they must
     stay equal; the whole launch being one colour is the point. **Verified gone** on all three tabs.
  **Timings here are a pessimistic bound, not the shipped numbers.** `android-build.sh` defaults to
  `OMNI_BUILD_MEM_SAFE=1`, and those `CARGO_PROFILE_RELEASE_*` exports are inherited by the
  `beforeBuildCommand`, so the **wasm is built at opt-level=1 with no LTO too**, not just the native
  lib. (Inferred from cargo honouring profile env-var overrides in child processes — not separately
  measured. Confirm by diffing wasm size against an `OMNI_BUILD_MEM_SAFE=0` build.)
  **Still unverified:** desktop/webkit2gtk rendering of the animation; the fully-optimized CI
  binary's real timings; and **whether 6s is the right cap — it is no longer reached on this device
  for any tab, so nothing exercises it.** Also unjudged by machine: animation smoothness and whether
  144px reads right in the hand — screen-grabs cannot answer either. [USER]
