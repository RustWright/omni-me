...
> `AuthState` model, the `POST /auto_import/reauth` route, and the `northwind-helper` `reauth` handler (server
...
> the real Northwind account:** a live TOTP code flipped `auth_state → active` and the next manual pull came back
> `last_outcome: success` / `health: healthy` (proving the session refreshed, not just the flag cleared).
> This **unblocks task 2.5** (Northwind auto-import may deploy to the VPS once Phase 2 lands). Motivating
> consumer: the Northwind auto-import source (private overlay).

## Problem

Auto-import runs **server-side** (the LLM key must live server-side; the Northwind subprocess + its
session file live wherever the server runs). The Northwind source authenticates with a
...
| The actual login/OTP protocol (driver invocation, exit-5 semantics) | — | ✅ Northwind-specific |

The public engine learns one new generic idea: **a source may be re-auth-capable and report an
auth state**. It still names no banks. Northwind is merely the first source that implements the private
half.
...
Northwind sends the code **in response to a login attempt**, so re-auth is inherently two-step
(trigger → receive → submit). Two shapes:

- **(a) Stateless two-call.** `start` runs a driver login(otp=null) purely to make Northwind send the
  ...
  **Risk:** if Northwind sends a *new* SMS on the `submit` attempt and rejects the `start` code, this breaks
...
**independent of any login attempt** — Northwind sends nothing. So there is nothing to "trigger," and the
...
- The client UI is just: `NeedsReauth` → "Reconnect Northwind" → an OTP field ("enter your
...
blip"), `auth_state` is imperative ("the user must act"). A degraded-but-active source (e.g. a Meridian PDF
...
- **Phase 2 (VPS deploy)** was **blocked on this** — going headless-on-VPS without it reintroduces the
  SSH-to-OTP problem. **Unblocked 2026-06-17:** app-entered re-auth has shipped, so Northwind auto-import may
  deploy to the VPS once the deploy pipeline (Phase 2) lands.
- The **local terminal prime** is no longer needed once deployed — the app reconnects Northwind itself.

## Open questions

1. ~~Is this Northwind account's 2FA TOTP or SMS?~~ **Resolved 2026-06-14: TOTP** → single-step `submit`,
