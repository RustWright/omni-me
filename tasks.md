...
- **DURABILITY guardrails (user Option 1, 2026-07-05) — the LLM-primary-interface push will keep the grammar churning, so these are load-bearing.** ✅ done: byte-chunk unit test; the canonical-source reconcile is proven manually. **QUEUED (next focused pass):** (1) **CI golden-reconcile** — import a *synthetic* fixture ledger (fictional banks) in CI and assert projected balances == `ledger bal` per-account-per-commodity (catches parser/grammar drift automatically); (2) **single canonical event-builder** in core that both the app commands AND the importer call, so "transaction syntax" lives in exactly one place (the drift between two builders caused the device_id + journal-key bugs); (3) **sync orphan-device-id self-check**. Post-v1: event schema-versioning + upcasting / migration-as-events (eliminate wipe+reimport when grammar changes).
