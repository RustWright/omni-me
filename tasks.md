...
- [ ] **2.3** Deploy pipeline — **NOW DESIGNED (see the Phase 2 plan); build it in the follow-up pipeline session.** Homed in the private overlay (Model A): build the real image → push to GHCR → deploy over the tailnet → health-gate + auto-rollback → pre-deploy snapshot → parametric target (DigitalOcean trial now / Hetzner long-term). [—]
