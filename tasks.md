...
**Next session — app delivery + CI/CD (queued 2026-06-28):**
- [ ] App delivery pipeline for **all targets** (desktop + Android) + a **low-friction wireless update path** to the devices that need it (no physical-cable/adb loop) — design the in-place update/refresh process, not just first install. The server CD (2.3b) is done; this is the *app* side. [L]
- [ ] Safe **production-box DB reset** for polish-phase testing — wipe/restore the box's data volume to a known state (lean on `deploy/snapshot.sh` / `restore-snapshot.sh`) so heavy test churn doesn't poison real data; **one final reset before v1 release**. [M]
- [ ] Wire the private overlay's **per-source account maps** so the account-map-based bank pollers actually emit drafts (they import 0 until wired — the private half of 3.9; receipt/email sources already work). [M]
