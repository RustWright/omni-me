...
- **REMAINING (Session 6, Steps 4–5):**
  - **Auto-sync never fires** — separate subsystem. Root cause confirmed: `append_and_apply` (`commands/shared.rs`) **never feeds the `SyncBuffer`**, so the `PushDebouncer` never wakes on an edit; **and there is no pull scheduler at all**. Fix = feed the buffer on local append (wakes the pusher) + add a lightweight periodic/online-triggered **pull** loop. (`core/src/sync/{pusher,accelerator}.rs`, `lib.rs`, `shared.rs`)
  - **Fresh-device backfill + cold-open hang** (the SEVERE-UX item below) — make first-run backfill non-blocking + instrument the journal read to root-cause the hang for real (the tasks.md "backfill locks the DB" note is UNVERIFIED — nothing auto-pulls today, so it can't be the current cause). Read-side UI reactivity (does the receiver refetch after a pull?) is a secondary follow-up.

**SEVERE UX:**
- [ ] **Journal stuck on "Loading…" on cold first-open** (reopen fixes it). **DEEPER THAN FIRST THOUGHT — links to sync backfill; carry into the sync session.** Reproduced precisely via uninstall→reinstall→first-open (the *only* way — a warm relaunch hides it): the journal hangs on the **data-fetch** "Loading…" state indefinitely (2min+ observed), a relaunch loads fully in ~8s. NOT the editor-init poll ("Initializing editor environment…" is a separate indicator; that fix is unrelated).
...
- [ ] **Desktop control contrast unreadable** in Settings — specifically the **default-currency picker** and the **LLM-provider selector** (checkbox/select controls). Low contrast against the dark theme; hard to read state. *User has 2 screenshots.* [S]
