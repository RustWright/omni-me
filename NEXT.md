...
- **SSH the box on its PROVIDER public IP as root, NOT over the tailnet** (user; how past
  sessions did it) — verified working. Address + invocation live in the private overlay's
  `deploy/` docs and past session logs; **they do not belong in this repo, which is public.** The
  *tailnet* SSH path is the refused one; tailnet HTTP on `:3000` is fine.
- **Box inventory (live, 2026-08-31):** one container `omni-me-private` (healthy), **one** volume
  `omni-deploy_omni_data`, **3** snapshots in `/var/omni-snapshots/` (~64 MB). `tasks.md`'s list
  is stale — the 87-byte no-op tarballs and the stray empty `omni_data` volume are already gone.
  A wipe needs the `OMNI_VOLUME` override + size check ([[project-hetzner-db-reset-for-testing]]).
