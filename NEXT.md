...
- ✅ **Public-repo identity scrub AUTHORISED** (2026-09-26), ⛔ only at the END of a session. Sites + traps: `tasks.md` § Owed at cycle close.

## ⛔ Inherit — settled, do not re-derive
- 🔴 **`gmail_work` and `yahoo` are STILL PAUSED**; only `gmail_personal` was resumed. ⚠️ A paused source reports `health: healthy` while importing nothing.
...
- ✅ **`ShippingNotice` is charge-bearing BY DESIGN** — vendors restate the total in it. ⛔ Do not "fix" the delivered mail being labelled `shipping_notice`.
...
- ⚠️ **No deploy has exercised the paged replay** — no version bump since `a0ad51a`. ✅ Dev builds
  dispatchable: ids `359907704` (image) / `359972206` (APK); `303640807` is LIVE and blocked.
