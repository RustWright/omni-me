...
- **The branch gate is safety-rails, deliberately NOT PR-gating (user, 2026-08-31).** Public repo
  ...
  rule gates DIRECT pushes too** — verified, it rejected a plain push to `main` — so it was removed;
  re-adding it breaks `session-end.sh` every session. The overlay can't be gated at all (free plan
  → `403 Upgrade to GitHub Pro`); it uses a tracked `pre-push` hook instead.
...
- Brokerage source needs an OTP Reconnect. Box runs **unauthenticated** — decide before the personal
  phone. Privacy guard can't catch "wise"; hand-grep public commits.
